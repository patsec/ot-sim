...
static int ini_handler(void *user, const char *section, const char *name, const char *value) {
  ...
  if (MATCH("proxy", "verbose")) {
    pconfig->verbose = atoi(value);
  } else if (MATCH("proxy", "pull")) {
    pconfig->pull = strdup(value);
  } else if (MATCH("proxy", "pub")) {
    pconfig->pub = strdup(value);
  } else if (MATCH("proxy", "debug")) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2) {
    ...
    /*
    if (ini_parse(argv[1], ini_handler, &c) < 0) {
      puts("cannot load config");
      return 1;
    }
    */
  }
