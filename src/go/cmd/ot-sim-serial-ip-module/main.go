...
import (
	...
	// This will cause the Serial-IP module to register itself with the otsim
	// package so it gets run by the otsim.Start function below.
	_ "github.com/patsec/ot-sim/serialip"
)
...
func main() {
	...
	if err := otsim.Start(ctx); err != nil {
		fmt.Printf("Error starting serial-IP module: %v\n", err)
	...
	if err := ctx.Err(); err != nil && !errors.Is(err, context.Canceled) {
		fmt.Printf("Error running serial-IP module: %v\n", err)
