...
import (
	...
	// This will cause the MQTT module to register itself with the otsim package
	// so it gets run by the otsim.Start function below.
	_ "github.com/patsec/ot-sim/mqtt"
)
...
func main() {
	...
	if err := otsim.Start(ctx); err != nil {
		fmt.Printf("Error starting MQTT module: %v\n", err)
	...
	if err := ctx.Err(); err != nil && !errors.Is(err, context.Canceled) {
		fmt.Printf("Error running MQTT module: %v\n", err)
